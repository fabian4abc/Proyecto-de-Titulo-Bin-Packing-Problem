...
public class D1_2 : MonoBehaviour
{
    private float posX;
    private float posY;
    private float posZ;
    ...
    public void OverBox(int cont)
    {
        ...
        if (Ocupado() <= (15 * 27.3 * 0.65))
        {
            ...
            Collider2D[] figures = Physics2D.OverlapBoxAll(new Vector2(6.825f, 3.75f), new Vector2(13.65f, 7.5f), 0);
            ...
            figures = Physics2D.OverlapBoxAll(new Vector2(-6.825f, 3.75f), new Vector2(13.65f, 7.5f), 0);
            foreach (object o in figures)
            {
                Collider2D g = (Collider2D)o;
                if (g.GetType() == typeof(PolygonCollider2D))
            ...
            if (Min == A1)
            {
                var b = Instantiate(Polx[cont], new Vector3(6.825f, 3.75f, 0f), Quaternion.identity);
