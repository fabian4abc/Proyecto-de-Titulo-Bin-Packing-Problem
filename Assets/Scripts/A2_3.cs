...
public class A2_3 : MonoBehaviour
{
    private float posX;
    private float posY;
    private float posZ;
    ...
    //Needs to update if more figures
    ...
    public void CambiarFigura(int cont) {
        if (Ocupado() <= (63.13 * 40 * 0.65) && cont > 1) {
            GameObject a = GameObject.Find(nombres[cont-1].ToString());
            int NGO;

            if (a != null)
            {
                NGO = int.Parse(a.name.Substring(5, 1));
                if (NGO != numGO - 1)
                {
                    var b = Instantiate(Polx[NGO+1], a.transform.GetChild(0).transform.position, Quaternion.identity);
                    b.name = a.name.Substring(0, 5)+NGO+1+")" + (2 * num_figuras - cont);
                    Destroy(a);
                }
            }
        } else
