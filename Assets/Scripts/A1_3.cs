...
public class A1_3 : MonoBehaviour
{
    private float posX;
    private float posY;
    private float posZ;
    ...
    public void RepetirFigura(int cont) {
        if (Ocupado() <= (63.13 * 40 * 0.75) && cont > 1) {
            GameObject a = GameObject.Find(nombres[cont-1].ToString());
            if (a != null)
            {
                var b = Instantiate(a);
                b.name = a.name.Substring(0, 7) + (2 * num_figuras - cont);
            }
        } else
        {
            accion++;
