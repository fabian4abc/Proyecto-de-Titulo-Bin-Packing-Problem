...
public class D1 : MonoBehaviour
{
    ...
    public void OverBox(int cont)
    {
        ...
        if (Ocupado() <= (59 * 40 * 0.65))
        {
            ...
            Collider2D[] figures = Physics2D.OverlapBoxAll(new Vector2(15, 10), new Vector2(30, 20), 0);
            ...
            figures = Physics2D.OverlapBoxAll(new Vector2(-15, 10), new Vector2(30, 20), 0);
            foreach (object o in figures)
            {
                Collider2D g = (Collider2D)o;
                if (g.GetType() == typeof(PolygonCollider2D))
            ...
            if (Min == A1)
            {
                var b = Instantiate(Polx[cont], new Vector3(15f, 10f, 0f), Quaternion.identity);
