...
public class D1_3 : MonoBehaviour
{
    private float posX;
    private float posY;
    private float posZ;
    ...
    public void BorrarAfuera()
    {
        ...
        // BORARA ESTE IF/ELSE Y PONER GUARDAR() EN IMPRIMIR SI SOLO QUIERO ULTIMA ITERACIÓN
        int Occ2 = Ocupado();
        if (accion == 3)
        {
            ScreenShot();
    ...
    public void OverBox(int cont)
    {

        if (Ocupado() <= (63.13 * 40 * 0.75))
        {
            int A1 = 0;
            int A2 = 0;
            int A3 = 0;
            ...
            Collider2D[] figures = Physics2D.OverlapBoxAll(new Vector2(15.75f, 10), new Vector2(31.5f, 20), 0);
            foreach (object o in figures)
            {
                Collider2D g = (Collider2D)o;
                if (g.GetType() == typeof(PolygonCollider2D))
                {
                    A1 += (int)g.GetComponent<Rigidbody2D>().mass;
                }
            }
            figures = Physics2D.OverlapBoxAll(new Vector2(-15.75f, 10), new Vector2(31.5f, 20), 0);
            foreach (object o in figures)
            {
                Collider2D g = (Collider2D)o;
                if (g.GetType() == typeof(PolygonCollider2D))
                {
                    A2 += (int)g.GetComponent<Rigidbody2D>().mass;
                }
            }
            figures = Physics2D.OverlapBoxAll(new Vector2(15.75f, -10), new Vector2(31.5f, 20), 0);
            foreach (object o in figures)
            {
                ...
                if (g.GetType() == typeof(PolygonCollider2D))
                {
                    A3 += (int)g.GetComponent<Rigidbody2D>().mass;
                }
            }
            figures = Physics2D.OverlapBoxAll(new Vector2(-15.75f, -10), new Vector2(31.5f, 20), 0);
            foreach (object o in figures)
            {
                ...
                if (g.GetType() == typeof(PolygonCollider2D))
                {
                    A4 += (int)g.GetComponent<Rigidbody2D>().mass;
                }
            }

            float Min = Mathf.Min(A1, A2, A3, A4);

            if (Min == A1)
            {
                var b = Instantiate(Polx[cont], new Vector3(15.75f, 10f, 0f), Quaternion.identity);
    ...
    public void Imprimir()
    {
        ...
        if (iteracion == 0)
        {
            string header = "Pol0\tPol1\tPol2\tPol3\tPol4\tPol5\tPol6\tPol7";
    ...
    public void Correr(string prog)
    {
        //Run the .exe file
        ...
        MP.StandardInput.WriteLine(prog);
        MP.StandardInput.Flush();
        MP.Close();
